 ...
 */
Migration.generate = function generate(migrationsDir) {
  var template = fs.readFileSync(__dirname + '/migration.template');
  var repl = {
    title: 'my human readable title of change',
    time: new Date().getTime(),
    id: guid()
  };
  var content = interpolate(template, repl);
  var filename = path.join(migrationsDir, repl.id + '.js');
