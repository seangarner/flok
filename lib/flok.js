...
const NOT_V1_MIGRATION = Symbol('NOT_V1_MIGRATION');
  ...
  async.whilst(
    ...
    function doExecute(doneOne) {
      async.auto({
        migration: function (cb) {
          self._run(self, self.dequeue, [pending], function (err, m) {
            ...
            if (m.flokVersion && !startsWith(m.flokVersion, '1')) {
              return cb(NOT_V1_MIGRATION, m);
            }
      ...
      function (err, res) {
        ...
        if (err === NOT_V1_MIGRATION) {
          self.log.warn('migration skipped; v' + m.flokVersion + ' incompatible with this flok');
          return doneOne(null);
        }
