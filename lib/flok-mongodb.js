...
function cli(opts) {
  ...
  return function onCli(program) {
    ...
    if (opts.indexOf('state') !== -1) {
      ...
      program.option('--mongodbStateColl <collection>',
                     'name of collection used to persist migration state');
    }
...
// generator mongodb sugar
module.exports['flok-init'] = function (program) {
  var state = cli(['state']);
  return function onFlokInit(flok) {
    state(program);
    flok.generate
