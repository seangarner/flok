  ...
  return function clearMigrationStatus(migration, done) {
    ...
    async.waterfall([
      ...
      function removeStatus(coll, callback) {
        coll.remove({_id: migration.status.id}, {}, callback);
      }
    ],
    function (err) {
      done(err);
...
function _unlock(url, collection) {
  return function unlock(done) {
    ...
    async.auto({
      ...
      lock: ['collection', function (callback, res) {
        res.collection.findOne({_id: 'flok_lock'}, {}, callback);
      }],

      remove: ['lock', function (callback, res) {
        res.collection.remove({_id: 'flok_lock'}, function (err, count) {
          if (err) return callback(err);
          if (isNaN(count) || count < 1) {
            if (res.lock && res.lock._id) self.log.warn('expected to clear lock but did not');
              else self.log.warn('no lock was present to clear');
            return callback(null);
          }
          self.log.info(res.lock, 'removed lock from host %s pid %s', res.lock.hostname,
                        res.lock.pid);
          callback(err);
        });
      }]
...
module.exports.onParse = function onParse(program, flok) {
  ...
  flok.generate
    .up('var collection = mig.mongodb.collection`(\'myCollection\');')
    .down('var collection = mig.mongodb.collection(\'myCollection\');');
};